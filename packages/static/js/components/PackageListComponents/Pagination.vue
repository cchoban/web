<template>
  <div aria-label="Pagination Navigation" role="navigation" class="ui pagination pointing secondary menu">
    ...
    <a class="item" :class="{'active': pages == currentPage}" v-for="(num, pages) in countPageNumber" @click="getPackages(`${store.state.api_urls.packages}/?limit=${maxentry}&offset=${num+extra_queries}`, pages)">{{ pages }}</a>
